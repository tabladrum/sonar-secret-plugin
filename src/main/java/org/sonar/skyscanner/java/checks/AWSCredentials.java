...
public class AWSCredentials extends IssuableSubscriptionVisitor {
    ...
    public void visitNode(Tree tree) {
        ...
        if (tree.is(Kind.VARIABLE)) {
            ...
            if (obj.initializer() != null && Objects.requireNonNull(obj.initializer()).is(Kind.STRING_LITERAL)) {
            ...
            obj.arguments().stream().filter(arg -> arg.is(Kind.STRING_LITERAL)).forEach(arg -> validate(tree, "", ((LiteralTree) arg).value()));
        } 
